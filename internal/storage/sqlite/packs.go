...
var _ packs.PackStore = (*SQLitePackStore)(nil)

// SQLitePackStore implements packs.PackStore backed by the SQLite provider.
type SQLitePackStore struct {
	db *sql.DB
}

// NewPackStore returns a PackStore backed by db.
func NewPackStore(db *sql.DB) *SQLitePackStore {
	return &SQLitePackStore{db: db}
}
...
func (s *SQLitePackStore) CreateSeed(ctx context.Context, seed packs.Seed) (packs.Seed, error) {
	id, err := newPackID()
	if err != nil {
	...
	_, err = s.db.ExecContext(ctx,
		...
		seed.TokenEstimate, string(tagsJSON), seed.AuthorRole,
...
func (s *SQLitePackStore) GetSeed(ctx context.Context, artifactID string) (packs.Seed, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, artifact_id, name, version_label, seed_type, role_access_bits,
		        description, content, token_estimate, tags, author_role, created_at
		 FROM seeds WHERE artifact_id = ?`,
		artifactID,
	)
...
func (s *SQLitePackStore) GetSeedByName(ctx context.Context, name string) (packs.Seed, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, artifact_id, name, version_label, seed_type, role_access_bits,
		        description, content, token_estimate, tags, author_role, created_at
		 FROM seeds WHERE name = ? ORDER BY created_at DESC LIMIT 1`,
		name,
	)
	seed, err := scanSeed(row)
	if errors.Is(err, sql.ErrNoRows) {
...
func (s *SQLitePackStore) ListSeeds(ctx context.Context, filter packs.SeedFilter) ([]packs.Seed, error) {
	query := `SELECT id, artifact_id, name, version_label, seed_type, role_access_bits,
		             description, content, token_estimate, tags, author_role, created_at
		      FROM seeds WHERE 1=1`
	args := []any{}
	if filter.SeedType != "" {
		query += " AND seed_type = ?"
	...
	for rows.Next() {
		var (
			id, artifactID, name, seedType, description, authorRole string
			versionLabel, tagsStr, contentStr                       string
		...
		if err := rows.Scan(
			...
			&nullDescription, &contentStr, &tokenEstimate, &tagsStr, &nullAuthorRole, &createdAtStr,
		); err != nil {
			return nil, fmt.Errorf("scan seed row: %w", err)
		}
		seed, err := buildSeed(id, artifactID, name, nullVersionLabel.String, seedType,
			roleAccessBits, nullDescription.String, contentStr, tokenEstimate, tagsStr,
			nullAuthorRole.String, createdAtStr)
		if err != nil {
			return nil, err
		}
...
// DeleteSeed tombstones a seed by removing its record.
// Soft delete is approximated by deletion since seeds are backing artifacts.
func (s *SQLitePackStore) DeleteSeed(ctx context.Context, artifactID string) error {
	_, err := s.db.ExecContext(ctx, `DELETE FROM seeds WHERE artifact_id = ?`, artifactID)
	if err != nil {
...
func (s *SQLitePackStore) CreatePack(ctx context.Context, pack packs.Pack) (packs.Pack, error) {
	id, err := newPackID()
	if err != nil {
	...
	_, err = s.db.ExecContext(ctx,
		...
		pack.CreatedAt.UTC().Format(time.RFC3339Nano),
	)
	if err != nil {
...
func (s *SQLitePackStore) GetPack(ctx context.Context, artifactID string) (packs.Pack, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, artifact_id, name, version_label, extends_id, role_bits, role_label,
		        description, pack_context, seeds, token_estimate, tags, author_role, created_at
		 FROM packs WHERE artifact_id = ?`,
		artifactID,
	)
	p, err := scanPack(row)
	if errors.Is(err, sql.ErrNoRows) {
...
func (s *SQLitePackStore) GetPackByName(ctx context.Context, name string) (packs.Pack, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, artifact_id, name, version_label, extends_id, role_bits, role_label,
		        description, pack_context, seeds, token_estimate, tags, author_role, created_at
		 FROM packs WHERE name = ? ORDER BY created_at DESC LIMIT 1`,
		name,
	)
	p, err := scanPack(row)
	if errors.Is(err, sql.ErrNoRows) {
...
func (s *SQLitePackStore) ListPacks(ctx context.Context, filter packs.PackFilter) ([]packs.Pack, error) {
	query := `SELECT id, artifact_id, name, version_label, extends_id, role_bits, role_label,
		             description, pack_context, seeds, token_estimate, tags, author_role, created_at
		      FROM packs WHERE 1=1`
	args := []any{}
	if filter.Role > 0 {
		query += " AND role_bits = ?"
	...
	for rows.Next() {
		var (
			id, artifactID, name, seedsStr, tokenEstStr, tagsStr, createdAtStr string
			roleBits, _ = 0, 0
		...
		if err := rows.Scan(
			...
			&nullDescription, &nullPackContext, &seedsStr, &tokenEstStr, &tagsStr, &nullAuthorRole, &createdAtStr,
		); err != nil {
			return nil, fmt.Errorf("scan pack row: %w", err)
		}
		p, err := buildPack(id, artifactID, name, nullVersionLabel.String, nullExtendsID.String,
			roleBits, nullRoleLabel.String, nullDescription.String, nullPackContext.String,
			seedsStr, tokenEstStr, tagsStr, nullAuthorRole.String, createdAtStr)
		if err != nil {
			return nil, err
		}
...
func (s *SQLitePackStore) DeletePack(ctx context.Context, artifactID string) error {
	_, err := s.db.ExecContext(ctx, `DELETE FROM packs WHERE artifact_id = ?`, artifactID)
	if err != nil {
...
func (s *SQLitePackStore) RecordTokenUsage(ctx context.Context, usage packs.TokenUsage) error {
	id, err := newPackID()
	if err != nil {
		return err
	}
	...
	_, err = s.db.ExecContext(ctx,
		...
		usage.TokenCount, approx, usage.ModelHint,
		usage.RecordedAt.UTC().Format(time.RFC3339Nano),
	)
	if err != nil {
...
func (s *SQLitePackStore) GetTokenUsage(ctx context.Context, filter packs.TokenFilter) (packs.TokenUsageSummary, error) {
	query := `SELECT phase, task, token_count, approximate FROM token_usage WHERE project = ?`
...
// scanSeed scans a single seed row from a *sql.Row.
func scanSeed(row *sql.Row) (packs.Seed, error) {
	var (
		id, artifactID, name, seedType, contentStr, tagsStr, createdAtStr string
		roleAccessBits, tokenEstimate                                      int
		nullVersionLabel, nullDescription, nullAuthorRole                  sql.NullString
	)
	if err := row.Scan(
	...
	return buildSeed(id, artifactID, name, nullVersionLabel.String, seedType,
		roleAccessBits, nullDescription.String, contentStr, tokenEstimate, tagsStr,
		nullAuthorRole.String, createdAtStr)
}

func buildSeed(id, artifactID, name, versionLabel, seedType string,
	roleAccessBits int, description, contentStr string, tokenEstimate int,
	tagsStr, authorRole, createdAtStr string) (packs.Seed, error) {
	...
	if err != nil {
		return packs.Seed{}, fmt.Errorf("parse seed created_at: %w", err)
	}
	return packs.Seed{
		ID:             id,
...
// scanPack scans a single pack row from a *sql.Row.
func scanPack(row *sql.Row) (packs.Pack, error) {
	var (
		id, artifactID, name, seedsStr, tokenEstStr, tagsStr, createdAtStr string
		roleBits                                                            int
		nullVersionLabel, nullExtendsID, nullRoleLabel                     sql.NullString
	...
	return buildPack(id, artifactID, name, nullVersionLabel.String, nullExtendsID.String,
		roleBits, nullRoleLabel.String, nullDescription.String, nullPackContext.String,
		seedsStr, tokenEstStr, tagsStr, nullAuthorRole.String, createdAtStr)
}

func buildPack(id, artifactID, name, versionLabel, extendsID string,
	roleBits int, roleLabel, description, packContext string,
	seedsStr, tokenEstStr, tagsStr, authorRole, createdAtStr string) (packs.Pack, error) {
	...
	createdAt, err := time.Parse(time.RFC3339Nano, createdAtStr)
	if err != nil {
		return packs.Pack{}, fmt.Errorf("parse pack created_at: %w", err)
	}
	return packs.Pack{
		ID:            id,
		ArtifactID:    artifactID,
