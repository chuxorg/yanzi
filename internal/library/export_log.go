...
type exportLogItemType string

const (
	exportLogItemCheckpoint exportLogItemType = "checkpoint"
	exportLogItemCapture    exportLogItemType = "capture"
	exportLogItemMeta       exportLogItemType = "meta"
)

type exportLogItem struct {
	Kind      exportLogItemType
	Timestamp string
	RowID     int64
...
func RenderOperationalExportLog(ctx context.Context, provider storage.Provider, project, cliVersion string, now time.Time, format ExportLogFormat, metaFilters map[string]string, includeDeleted bool) ([]byte, string, error) {
	...
	items := make([]exportLogItem, 0, len(storageItems))
	for _, item := range storageItems {
		items = append(items, exportLogItemFromStorage(item))
	}

	switch format {
	case ExportLogFormatMarkdown:
		return []byte(renderMarkdownLog(project, cliVersion, now, items, captureCount)), "text/markdown; charset=utf-8", nil
	case ExportLogFormatJSON:
		content, err := renderJSONLog(project, cliVersion, now, items)
		return content, "application/json; charset=utf-8", err
	case ExportLogFormatHTML:
		return []byte(renderHTMLLog(project, cliVersion, now, items)), "text/html; charset=utf-8", nil
	default:
		return nil, "", fmt.Errorf("unsupported export format: %s", format)
	}
}

func exportLogItemFromStorage(item storage.ExportItem) exportLogItem {
	out := exportLogItem{
...
func renderMarkdownLog(project, cliVersion string, now time.Time, items []exportLogItem, captureCount int) string {
...
func renderJSONLog(project, cliVersion string, now time.Time, items []exportLogItem) ([]byte, error) {
	events := make([]any, 0, len(items))
	for _, item := range items {
		switch item.Kind {
		case exportLogItemCheckpoint:
			events = append(events, jsonCheckpointEvent{
				Type:      string(exportLogItemCheckpoint),
		...
		default:
			events = append(events, jsonCaptureEvent{
				Type:      string(exportLogItemCapture),
				ID:        item.CaptureID,
				Role:      item.Role,
				Timestamp: item.Timestamp,
				Hash:      item.Hash,
...
func renderHTMLLog(project, cliVersion string, now time.Time, items []exportLogItem) string {
	...
	for idx, item := range items {
		...
		if item.Kind == exportLogItemCheckpoint {
			entryClass += " timeline-entry-checkpoint"
		} else if item.Kind == exportLogItemMeta {
...
func exportSearchText(item exportLogItem) string {
...
func checkpointBadges(item exportLogItem) []string {
	return []string{"Checkpoint", "Boundary", "Rehydration Anchor", "Hash"}
}

func captureBadges(item exportLogItem) []string {
...
func isRuleArtifact(item exportLogItem) bool {
